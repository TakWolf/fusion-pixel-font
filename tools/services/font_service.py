...
class FontBuildContext:
    ...
    def load(font_size: FontSize) -> FontBuildContext:
        ...
        named_scope_contexts = {
            glyph_scope: NamedContext().merge_by_name_key(
                NamedContext.load(
                    path_define.ARK_PIXEL_GLYPHS_DIR.joinpath(str(font_size), 'named', glyph_scope),
                    allowed_flavors=options.LANGUAGE_FLAVORS,
                ),
                NamedContext.load(
                    path_define.PATCH_GLYPHS_DIR.joinpath(str(font_size), 'named', glyph_scope),
                    allowed_flavors=options.LANGUAGE_FLAVORS,
                ),
                conflict='replace',
            )
    ...
    def create_builder(self, width_mode: WidthMode, language_flavor: LanguageFlavor) -> FontBuilder:
        ...
        builder.meta_info.copyright_info = 'Copyright (c) 2022, TakWolf (https://takwolf.com)'
