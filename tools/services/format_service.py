...
def normalize_cmap_glyphs(font_size: FontSize) -> None:
    for glyph_scope in options.GLYPH_SCOPES:
        glyph_scope_dir = path_define.PATCH_GLYPHS_DIR.joinpath(str(font_size), 'cmap', glyph_scope)
...
def normalize_named_glyphs(font_size: FontSize) -> None:
    for glyph_scope in options.GLYPH_SCOPES:
        glyph_scope_dir = path_define.PATCH_GLYPHS_DIR.joinpath(str(font_size), 'named', glyph_scope)
...
def format_glyphs() -> None:
    if sys.platform != 'win32':
        fs_util.format_glyph_files(path_define.PATCH_GLYPHS_DIR)
